    ...
    int encrypt)
{
    ...
    while (processed_bytes < total_size)
    {
        ...
        for (size_t i = 0; i < num_sectors; i++)
        {
            ...
            size_t sector_size = (sector_offset + SECTOR_SIZE <= (size_t)bytesRead) ? SECTOR_SIZE : ((size_t)bytesRead - sector_offset);
            ...
            memcpy(tweak, &data_unit_number, sizeof(data_unit_number) < MAES_TWEAK_SIZE ? sizeof(data_unit_number) : MAES_TWEAK_SIZE);
