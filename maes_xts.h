...
#define BUFFER_SIZE                                                       \
  (8 * 1024 * 1024) /* 8 MB - velkost hlavneho buffra pre prenos dat */
#define SECTOR_SIZE                                                       \
  4096 /* Velkost jedneho sektora, pouziva sa na zarovnanie */
...
#define KDF_CONTEXT                                                       \
  "maes-xts-derive"           /* Kontextovy retazec pre BLAKE3 KDF */
...
#define HEADER_MAGIC                                                      \
  "MAESXTS" /* Magicky retazec pre identifikaciu hlavicky */
...
#define VERIFICATION_BLOCK_SIZE                                           \
  BLOCK_SIZE /* Velkost verifikacneho bloku (AES block size) */
#define VERIFICATION_PLAINTEXT                                            \
  "MAES_VERIFY_OK\0\0" /* Znamy plaintext pre verifikaciu (16 bytes) */

/* ========== Zobrazenie postupu ========== */
#define PROGRESS_UPDATE_INTERVAL                                          \
  10000 /* Ako casto aktualizovat zobrazenie postupu (v sektoroch) */
...
typedef struct {
  ...
  uint32_t
      key_bits; // Velkost kluca v bitoch (musi zodpovedat MAES_KEY_BITS)
  uint8_t salt[SALT_SIZE]; // Sol pre KDF (BLAKE3)
  // This should now use the correctly defined VERIFICATION_BLOCK_SIZE
  uint8_t
      verification_tag[VERIFICATION_BLOCK_SIZE]; /* Encrypted verification
                                                    block */
  uint8_t reserved[SECTOR_SIZE - HEADER_MAGIC_SIZE - 1 - 1 - 4 - 4 -
                   SALT_SIZE -
                   VERIFICATION_BLOCK_SIZE]; // Padding to fill sector
} maes_header_t;
#pragma pack(pop) // Obnovenie povodneho zarovnania

/**
 * Kontext zariadenia - obsahuje vsetky potrebne informacie pre manipulaciu
 * so zariadenim Ma rozne cleny v zavislosti od operacneho systemu
 */
typedef struct {
#ifdef _WIN32
